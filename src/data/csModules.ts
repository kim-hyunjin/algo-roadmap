    ...
    concepts: [
      {
        ...
        answers: ["일반적으로 최상위 비트에 부호를 따로 두지 않고 2의 보수로 표현해 덧셈 회로를 그대로 사용할 수 있게 합니다.", "UTF-8은 문자 코드 값에 따라 1~4바이트를 쓰는 가변 길이 인코딩이므로 문자마다 바이트 수가 다릅니다.", "대부분의 10진 소수는 이진수로 정확히 표현할 수 없어 연산 과정에서 반올림 오차가 생기기 때문입니다."],
    ...
    concepts: [
      {
        ...
        answers: ["프로세스끼리는 기본적으로 주소 공간이 분리되고, 같은 프로세스의 스레드는 코드·힙·파일 같은 자원을 공유하되 각자 스택과 레지스터 상태를 가집니다.", "실행 상태를 저장·복원하고 스케줄러가 개입하며 캐시와 TLB의 유효한 데이터가 줄어들 수 있기 때문입니다.", "동시성은 여러 작업이 겹치는 시간에 진전하도록 구성하는 것이고, 병렬성은 여러 작업을 실제로 동시에 실행하는 것입니다."],
      ...
      },
      {
        ...
        answers: ["스택 데이터는 보통 함수 호출 범위에 따라 자동으로 생성·해제되고, 힙 데이터는 동적으로 할당되어 명시적 해제나 가비지 컬렉션까지 살아 있습니다.", "프로세스가 접근한 가상 메모리 페이지가 현재 물리 메모리에 없거나 접근 권한이 맞지 않을 때 발생합니다.", "전체 읽기는 파일 크기만큼 메모리를 점유하지만 스트리밍은 작은 조각씩 처리해 메모리 사용량을 제한합니다."],
  ...
  },
  {
    ...
    concepts: [
      ...
      },
      {
        ...
        answers: ["TLS로 통신 내용을 암호화하고 변조 여부와 서버 신원을 확인하지만, 단말이나 서버 자체의 침해까지 막지는 못합니다.", "GET은 자원 조회를 뜻하며 안전하고 멱등이어야 하고, POST는 서버에 처리를 요청하거나 새 상태를 만드는 데 주로 씁니다.", "4xx는 요청·권한 등 클라이언트 측 원인, 5xx는 요청을 처리하지 못한 서버 측 원인을 나타냅니다."],
      ...
      },
      {
        ...
        answers: ["응답 없는 작업이 연결·스레드 같은 자원을 계속 점유해 대기가 누적되고 전체 서비스가 멈출 수 있습니다.", "이미 과부하인 서버에 요청이 더 몰리는 재시도 폭풍이 생겨 복구를 늦추고 장애를 확산할 수 있습니다.", "같은 작업의 재요청을 식별해 결제·주문·작업 생성 같은 부수 효과가 중복 실행되는 것을 막습니다."],
    ...
    concepts: [
      {
        ...
        answers: ["기본 키는 각 행의 고유 식별을, 외래 키는 참조하는 값이 대상 테이블에 존재한다는 참조 무결성을 보장합니다.", "지정한 결합 조건이 맞는 두 테이블의 행을 연결하며, 조인 종류에 따라 조건에 맞지 않는 행의 포함 여부가 달라집니다.", "정규화는 중복과 갱신 오류를 줄이지만 조인이 늘 수 있고, 비정규화는 조회를 단순화하는 대신 중복 데이터의 일관성 관리 비용을 만듭니다."],
      ...
      },
      {
        ...
        answers: ["한 작업의 일부 변경만 반영되는 것을 막아 묶인 변경이 모두 커밋되거나 모두 롤백되게 합니다.", "더티 리드, 반복 불가능 읽기, 팬텀 리드처럼 동시에 실행된 트랜잭션의 중간 상태나 변경이 관찰될 수 있습니다.", "원본이 바뀌는 모든 경로에서 복사본을 정확한 시점에 갱신·삭제해야 하며 분산 환경에서는 순서와 실패까지 고려해야 하기 때문입니다."],
    ...
    concepts: [
      {
        ...
        answers: ["사용자 신원 확인과 허용된 행동 판단은 별개의 정책이며, 인증된 사용자도 모든 자원에 접근할 수 있어서는 안 되기 때문입니다.", "저장소·로그·배포 산출물에 노출되기 쉽고 유출 뒤 회수와 교체도 어려우므로 별도 비밀 관리 시스템을 써야 합니다.", "신뢰 수준이 바뀌는 모든 경계, 특히 외부 입력이 시스템에 들어오거나 다른 시스템의 결과를 사용할 때 검증해야 합니다."],
      ...
      },
      {
        ...
        answers: ["질문·지침·검색 문서·출력이 컨텍스트 한도를 나눠 쓰므로 관련성이 높은 정보만 선택하고 길이를 관리해야 합니다.", "키워드 검색은 단어의 정확한 일치를 주로 보고, 임베딩 검색은 문장을 벡터로 표현해 의미상 가까운 내용을 찾습니다.", "도구도 실패하거나 오래된·악의적인 데이터를 반환할 수 있고 모델이 결과를 잘못 해석할 수 있으므로 형식·권한·내용을 검증해야 합니다."],
      ...
      },
      {
        ...
        answers: ["오프라인 평가는 고정된 데이터셋으로 출시 전 품질을 반복 비교하고, 온라인 지표는 실제 사용자 환경의 행동·성공률·장애를 관찰합니다.", "안전성, 일관성, 근거성, 비용, 지연 시간, 실패율과 사용자 만족도 등을 함께 측정해야 합니다.", "되돌리기 어렵거나 금전·법률·안전·개인정보에 큰 영향을 주는 고위험 작업에는 실행 전 사람의 승인이 필요합니다."],
